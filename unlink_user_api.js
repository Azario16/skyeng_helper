...
.then(json => {teacher_id=(json.user.id)
      ...
      fetch("https://rooms.vimbox.skyeng.ru/users/api/v1/teachers/"+teacher_id+"/students", {})
      .then(response => { if (response.ok === true) { return response.json(); } else { return null; } })
      .then(json => {a=(json.length) 
                        for (var i = 0; i < a; i++){
                          ...
                          console.log(userid)
                          //Запрос на удаление
                          fetch("https://rooms.vimbox.skyeng.ru/users/api/v1/teachers/unlink-student/"+ userid +"", {"method": "POST",});
                        }  
        })
})
